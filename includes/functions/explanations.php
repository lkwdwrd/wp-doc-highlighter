...
class WPORG_Explanations {
	...
	public function __construct() {
		$this->post_types = \WP_Doc\Reference\get_reference_types();
	...
	public function post_to_expl_controls( $post ) {
		...
		$explanation = WP_Doc\Legacy\get_explanation( $post );
	...
	public function admin_enqueue_scripts() {
		...
		if ( in_array( get_current_screen()->id, array_merge(
				\WP_Doc\Reference\get_reference_types(),
				array( 'wporg_explanations', 'edit-wporg_explanations' )
		) ) ) {
			wp_enqueue_style( 'wporg-admin', WPDOC_URL . 'assets/css/stylesheets/admin.css', array(), '20141218' );
			wp_enqueue_script( 'wporg-explanations', WPDOC_URL . 'assets/js/vendor/explanations.js', array( 'jquery', 'wp-util' ), '20141218', true );
	...
	public function new_explanation() {
		...
		if ( WP_Doc\Legacy\get_explanation( $post_id ) ) {
	...
	public function un_publish_explanation() {
		...
		if ( $explanation = \WP_Doc\Legacy\get_explanation( $post_id ) ) {
